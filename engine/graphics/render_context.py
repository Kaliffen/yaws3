...
class RenderContext:
    ...
    def update_camera(self, camera, planet_center=None, width=None, height=None):
        if width is not None:
            self.width = int(width)
        if height is not None:
            self.height = int(height)
        self.camera_position_world = camera.position.astype(np.float32)
        if planet_center is not None:
            self.planet_center_rel = np.array(planet_center, dtype=np.float32) - self.camera_position_world
        self.view = camera.get_view_matrix()
        self.projection = camera.get_projection_matrix()
        view_proj = np.matmul(self.projection, self.view)
        self.inv_view_proj = np.linalg.inv(view_proj).astype(np.float32)
